...
class NotificationItem(QWidget):
    ...
    def __init__(self, title, message, item, *args, ntype=0, callback=None, **kwargs):
        ...
        # 关闭按钮
        self.labelClose = QLabel(self, cursor=Qt.PointingHandCursor,
                                 pixmap=NotificationIcon.icon(NotificationIcon.Close))

        # 消息内容
        self.labelMessage = QLabel(message, self, cursor=Qt.PointingHandCursor, wordWrap=True,
...
class NotificationWindow(QListWidget):
    ...
    @classmethod
    def _createInstance(cls, parent):
        # 创建实例
        if not cls._instance:
            cls._instance = NotificationWindow(parent)
            cls._instance.show()
            NotificationIcon.init()

    @classmethod
    def info(cls, parent, title, message, callback=None):
        cls._createInstance(parent)
    ...
    @classmethod
    def success(cls, parent, title, message, callback=None):
        cls._createInstance(parent)
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, cls._instance,
    ...
    @classmethod
    def warning(cls, parent, title, message, callback=None):
        cls._createInstance(parent)
        item = QListWidgetItem(cls._instance)
        w = NotificationItem(title, message, item, cls._instance,
    ...
    @classmethod
    def error(cls, parent, title, message, callback=None):
        cls._createInstance(parent)
        item = QListWidgetItem(cls._instance)
